...
int main(int argc, char** argv) {
	...
	cerr << COLOR_BLUE << "************************************************************" << COLOR_NC << endl;
	cerr << COLOR_BLUE << "* [2] openCV JPEG decode and encode with openCV resize 1/2 *" << COLOR_NC << endl;
	cerr << COLOR_BLUE << "************************************************************" << COLOR_NC << endl;
	start=clock();
	for(int i=0; i<atoi(argv[1]); i++) {
		// JPEG to Mat
		...
		resize(imgMat, imgMat, Size(IMAGE_WIDTH/2, IMAGE_HEIGHT/2));
		// Mat to JPEG
		vector<uchar> buf;
		imencode(".jpg", imgMat, buf, std::vector<int>());
		#if ENABLE_WRITE_IN_B2 == 1
		write(STDOUT_FILENO, &buf[0], buf.size());
		#endif
	}
	end=clock();
	...
	cerr << COLOR_BLUE << "*************************************************************" << COLOR_NC << endl;
	cerr << COLOR_BLUE << "* [3] openCV JPEG decode and encode with openCV pryDown 1/2 *" << COLOR_NC << endl;
	cerr << COLOR_BLUE << "*************************************************************" << COLOR_NC << endl;
	start=clock();
	for(int i=0; i<atoi(argv[1]); i++) {
		// JPEG to Mat
		...
		pyrDown(imgMat, imgMat, Size(IMAGE_WIDTH/2, IMAGE_HEIGHT/2));
		// Mat to JPEG
		vector<uchar> buf;
		imencode(".jpg", imgMat, buf, std::vector<int>());
		#if ENABLE_WRITE_IN_B3 == 1
		write(STDOUT_FILENO, &buf[0], buf.size());
		#endif
	}
	end=clock();
